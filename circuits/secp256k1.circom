...
template K1_AddUnequalCubicConstraint() {
    signal input x1[4];
    signal input y1[4];
    signal input x2[4];
    signal input y2[4];
    ...
    component x1x22Comp = K1_A2B1NoCarry();
    ...
    component x22x3Comp = K1_A2B1NoCarry();
    ...
    component x12x3Comp = K1_A2B1NoCarry();
    ...
    component x1x2x3Comp = K1_A1B1C1NoCarry();
...
template K1_Secp256k1PointOnLine() {
    signal input x1[4];
    signal input y1[4];
...
template K1_Secp256k1PointOnTangent() {
    ...
    component y1y3Comp = K1_BigMultNoCarry(64, 64, 64, 4, 4);
    ...
    component zeroCheck = K1_CheckCubicModPIsZero(199);
    for (var i = 0; i < 10; i++) {
...
// x^3 + 7 - y^2 == 0 mod p
// where p is the secp256k1 field size
template K1_Secp256k1PointOnCurve() {
    signal input x[4];
    signal input y[4];
...
template K1_Secp256k1AddUnequal(n, k) {
    assert(n == 64 && k == 4);
    ...
    var tmp[2][100] = K1_secp256k1_addunequal_func(n, k, x1, y1, x2, y2);
    for(var i = 0; i < k;i++){
        out[0][i] <-- tmp[0][i];
        out[1][i] <-- tmp[1][i];
    }

    component cubic_constraint = K1_AddUnequalCubicConstraint();
    for(var i = 0; i < k; i++){
    ...
    component point_on_line = K1_Secp256k1PointOnLine();
    for(var i = 0; i < k; i++){
    ...
    component x_check_in_range = K1_CheckInRangeSecp256k1();
    component y_check_in_range = K1_CheckInRangeSecp256k1();
    for(var i = 0; i < k; i++){
...
template K1_Secp256k1Double(n, k) {
    assert(n == 64 && k == 4);
    ...
    var tmp[2][100] = K1_secp256k1_double_func(n, k, x1, y1);
    for(var i = 0; i < k;i++){
        out[0][i] <-- tmp[0][i];
        out[1][i] <-- tmp[1][i];
    }

    component point_on_tangent = K1_Secp256k1PointOnTangent();
    for(var i = 0; i < k; i++){
    ...
    component point_on_curve = K1_Secp256k1PointOnCurve();
    for(var i = 0; i < k; i++){
    ...
    component x3_eq_x1 = K1_BigIsEqual(4);
    for(var i = 0; i < k; i++){
...
template K1_Secp256k1ScalarMult(n, k) {
    ...
    for (var i = k - 1; i >= 0; i--) {
        for (var j = n - 1; j >= 0; j--) {
            ...
            if (i < k - 1 || j < n - 1) {
                adders[n * i + j] = K1_Secp256k1AddUnequal(n, k);
                doublers[n * i + j] = K1_Secp256k1Double(n, k);
                for (var idx = 0; idx < k; idx++) {
